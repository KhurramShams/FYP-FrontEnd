...
export default function SandboxPage() {
  ...
  useEffect(() => {
    const generatedId = `sandbox-${Math.random().toString(36).substring(2, 15) + Date.now().toString(36)}`;
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await api.post('/v1/web/query/portal_query', {
        ...
        unique_id: uniqueId || `session-${Date.now()}`
      });
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: 'calc(100vh - 120px)' }}>
      <div style={{ marginBottom: '24px' }}>
        <h1 style={{ fontSize: '32px', marginBottom: '8px' }}>Chat Sandbox</h1>
