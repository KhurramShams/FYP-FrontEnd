...
// ── Detail Drawer ─────────────────────────────────────────────────────────────
function Drawer({ apiKey, onClose, analyticsLevel }: { apiKey: ApiKey; onClose: () => void; analyticsLevel: string }) {
      ...
      <div style={{ position: 'fixed', top: 0, right: 0, bottom: 0, width: '480px', background: '#13131a', borderLeft: `1px solid ${THEME.border}`, zIndex: 50, overflowY: 'auto', padding: '32px', boxShadow: '-20px 0 40px rgba(0,0,0,0.5)' }}>
        ...
        <div style={{ marginBottom: '24px' }}>
          ...
          <div style={{ background: THEME.bgCard, border: `1px solid ${THEME.border}`, borderRadius: '12px', padding: '16px' }}>
            {analyticsLevel === 'No' ? (
              <p style={{ fontSize: '13px', color: THEME.textMuted, textAlign: 'center', margin: 0 }}>🔒 Locked under current plan.</p>
            ) : loadingUsage ? (
...
function ApiUsageCharts({ usageData, analyticsLevel }: { usageData: ApiUsageData | null; analyticsLevel: string }) {
  if (!usageData) return null;
  const totalTokens = usageData.total_prompt_tokens + usageData.total_completion_tokens;
  const pieData = [
      ...
      <div style={{ background: THEME.bgCard, border: `1px solid ${THEME.border}`, borderRadius: '16px', padding: '24px' }}>
        ...
        <div style={{ width: '100%', height: '200px' }}>
          {analyticsLevel === 'Basic' ? (
            <div style={{ width: '100%', height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', color: THEME.textMuted, fontSize: '12px', border: `1px dashed ${THEME.border}`, borderRadius: '12px', gap: '8px' }}>
              <span>🔒 Advanced Analytics Required</span>
              <a href="/app/subscriptions" style={{ fontSize: '11px', color: THEME.primary, textDecoration: 'none' }}>Upgrade Plan</a>
            </div>
          ) : totalTokens === 0 ? (
...
export default function ApiManagementPage() {
  ...
  const analyticsLevel = subscription?.analytics ?? 'No';
  ...
  useEffect(() => {
    ...
    fetchSubscriptionDetails(user.user_id).then(setSub);
  }, [ready, user?.user_id]);
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', maxWidth: '1400px', margin: '0 auto', gap: '8px' }}>
      ...
      {detailsKey && <Drawer apiKey={detailsKey} onClose={() => setDetailsKey(null)} analyticsLevel={analyticsLevel} />}

      <ErrorBanner error={error} onDismiss={() => setError('')} style={{ marginBottom: '24px' }} />
      ...
      {loadingUsage ? (
        ...
        <div style={{ height: '320px', background: THEME.bgCard, border: `1px dashed ${THEME.border}`, borderRadius: '16px', display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: '16px', padding: '40px', textAlign: 'center', marginTop: '24px' }}>
          <span style={{ fontSize: '28px' }}>🔒</span>
          <h3 style={{ fontSize: '16px', color: '#fff', fontWeight: 600, margin: 0 }}>API Analytics Feature Locked</h3>
          <p style={{ fontSize: '13px', color: THEME.textMuted, maxWidth: '360px', margin: 0, lineHeight: 1.5 }}>Upgrade your subscription plan to unlock detailed compute distribution charts, token monitor logs, and API latency stats.</p>
          <a href="/app/subscriptions" style={{ padding: '10px 20px', background: THEME.primary, border: 'none', borderRadius: '10px', color: '#fff', fontSize: '13px', fontWeight: 600, cursor: 'pointer', textDecoration: 'none', marginTop: '8px' }}>Upgrade Plan</a>
        </div>
      ) : selectedKey ? (
        <>
          <ApiStatsCards usageData={usageData} />
          <ApiUsageCharts usageData={usageData} analyticsLevel={analyticsLevel} />
        </>
      ) : (
