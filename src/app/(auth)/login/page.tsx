...
import { loginUser, saveSession, extractErrorMessage } from '@/lib/authService';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.status === 'success' && response.access_token) {
        // Backend now returns user_id + email directly — no extra /google/verify call needed.
        // Using the response data directly avoids the Supabase external API round-trip
        // that was rejecting newly-registered users whose email wasn't confirmed in Supabase Auth.
        const userId    = response.user_id   ?? '';
        const userEmail = response.email     ?? email;

        saveSession(response.access_token, userId, userEmail);

        // Redirect into the app (history replacement to prevent back-button loops)
